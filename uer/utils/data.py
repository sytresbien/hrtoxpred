...
class MlmDataset(Dataset):
    ...
    def build_instances(self, all_documents):
        ...
        # if len(src) < seq_len, then skip this for-loop
        ...
        while True:
            ...
            for ins in instances:
                if len(ins) == 3:
                    ...
                    # print("src_single: ", src_single)
                    # print("tgt_single: ", tgt_single)
...
class ChemMlmDataset(Dataset):
    def __init__(self, args, vocab, tokenizer):
        super(ChemMlmDataset, self).__init__(args, vocab, tokenizer)
        print("ChemMlmDataset...")
        self.full_sentences = args.full_sentences
        ...
        for _ in range(self.dup_factor):
            ...
            with open(self.corpus_path, mode="r", encoding="utf-8") as f:
                ...
                while True:
                    ...
                    # transfer words into tokens
                    # one line one document
                    document = [self.vocab.get(CLS_TOKEN)] + self.tokenizer.convert_tokens_to_ids(self.tokenizer.tokenize(line[0])) + [self.vocab.get(SEP_TOKEN)]
                    # print("atoms_len: ", atoms_len)
                    # generate atoms segment
                    segment = line[1].strip().split(" ")
                    seg = [0] + list(map(int, segment)) + [0]
                    # print("seg_num: ", len(seg))
                    if self.full_sentences:
                        if len(document) > 0:
                            docs_buffer.append(document)
                        if len(docs_buffer) == self.docs_buffer_size:
                        ...
                        if len(document) > 0:
                            instances = self.build_chem_instances(document, seg)
                            # Save instances.
                            for instance in instances:
                                pickle.dump(instance, dataset_writer)
    ...
    def build_chem_instances(self, all_documents, seg):
        instances = []
        instances_num = len(all_documents) // self.seq_length
        # if len(src) < seq_len, then skip this for-loop
        for i in range(instances_num):
            src = all_documents[i * self.seq_length: (i + 1) * self.seq_length]
            temp_seg = seg[i * self.seq_length: (i + 1) * self.seq_length]

            seg_pos = [len(src)]
            if not self.dynamic_masking:
                src, tgt = mask_seq(src, self.tokenizer, self.whole_word_masking, self.span_masking, self.span_geo_prob, self.span_max_length)
                instance = (src, tgt, temp_seg, seg_pos)
            else:
                instance = (src, temp_seg, seg_pos)

            if (len(instance[1]) == self.seq_length):
                instances.append(instance)
            else:
                print(instance)

        src = all_documents[instances_num * self.seq_length:]
        seg = seg[instances_num * self.seq_length:]
        seg_pos = [len(src)]

        # print("after for-loop seg: ", seg)
        ...
        if not self.dynamic_masking:
            src, tgt = mask_seq(src, self.tokenizer, self.whole_word_masking, self.span_masking, self.span_geo_prob, self.span_max_length)
            instance = (src, tgt, seg, seg_pos)
        else:
            instance = (src, seg, seg_pos)
...
class ChemMlmDataLoader(DataLoader):
    def __iter__(self):
        print("ChemMlmDataLoader ...")
        while True:
            while self._empty():
                self._fill_buf()
            if self.start + self.batch_size >= self.end:
            ...
            for ins in instances:
                ...
                for mask in tgt_single:
                    tgt[-1][mask[0]] = mask[1]
                seg.append(ins[1])
                # print("src len: ", len(src_single))
                # print("seg len: ", len(ins[1]))
                # print("src: ", src)
                # print("tgt: ", tgt)
                # print("seg: ", seg)
