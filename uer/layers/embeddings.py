...
# Word soft-position embedding for SMILES
class SmilesEmbedding(nn.Module):
    ...
    def __init__(self, args, vocab_size):
        ...
        self.word_embedding = nn.Embedding(vocab_size, args.emb_size)
        self.position_embedding = nn.Embedding(self.max_seq_length, args.emb_size)
        self.segment_embedding = nn.Embedding(5, args.emb_size)
        if not self.remove_embedding_layernorm:
            self.layer_norm = LayerNorm(args.emb_size)
...
class WordPosSegEmbedding(nn.Module):
    ...
    def __init__(self, args, vocab_size):
        ...
        print("Word_pos_seg embedding...")
        self.remove_embedding_layernorm = args.remove_embedding_layernorm
        self.dropout = nn.Dropout(args.dropout)
        self.max_seq_length = args.max_seq_length
        self.word_embedding = nn.Embedding(vocab_size, args.emb_size)
        self.position_embedding = nn.Embedding(self.max_seq_length, args.emb_size)
        self.segment_embedding = nn.Embedding(4, args.emb_size)
        if not self.remove_embedding_layernorm:
            self.layer_norm = LayerNorm(args.emb_size)
    ...
    def forward(self, src, seg):
        ...
        # segment embedding input the radius feature
        seg_emb = self.segment_embedding(seg)
